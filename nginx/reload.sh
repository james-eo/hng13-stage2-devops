...
# nginx reload script for runtime configuration changes
# This script regenerates the configuration and reloads nginx gracefully

CONF_FILE="/etc/nginx/conf.d/default.conf"
ACTIVE_POOL=${ACTIVE_POOL:-blue}
PORT=${PORT:-8080}

echo "Reloading nginx configuration for active pool: $ACTIVE_POOL"

# Function to generate nginx configuration (same as entrypoint)
generate_config() {
...
if [ $? -eq 0 ]; then
    # Graceful reload
    nginx -s reload
    echo "nginx reloaded successfully with new configuration"
    echo "Active pool is now: $ACTIVE_POOL"
else
